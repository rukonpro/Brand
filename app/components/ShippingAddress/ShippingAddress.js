...
export default function ShippingAddress({ toggleDrawer2, mutateDefualtShippingAdresss, openModal }) {
    ...
    const handleUpdateDefaultAddress = async () => {
        setLoading(true);
        const res = await updateDefaultShppingAddress_Api({ addressId: selectAddress, userId: user?.id });
        ...
        <div className="max-w-md mx-auto bg-white rounded-lg dark:bg-slate-800">
            ...
            <div className="mt-4 flex justify-end space-x-2">
                ...
                <button
                    ...
                    disabled={loading}
                    className={`px-4 py-2 bg-teal-500 text-white rounded hover:bg-teal-600 ${loading ? "opacity-20" : ""}`}>
                    Save
                </button>
            </div>
        </div>
