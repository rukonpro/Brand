...
const Drawer = ({isDrawerOpen, handleClose,children,destination }) => {
    ...
    return (
        <div className={`fixed inset-0 z-10 overflow-hidden  duration-700  ${isDrawerOpen ? 'block' : 'hidden'}`}>
            ...
            <div className={`fixed inset-y-0 ${destination}-0 flex max-w-xs w-full bg-blue-200 dark:bg-slate-800  shadow-xl transform transition ease-in-out duration-300 overflow-y-auto`}>
                {children}
            </div>
        </div>
    );
