...
const NestedDropdown = ({ category,setCategoryId,categoryId }) => {
        ...
        <li className="relative group  w-full">

            {
                category?.children?.length === 0 || !category?.children ?(
                            ...
                            <button onClick={()=>setCategoryId(category?.id)} className={`${category?.id===categoryId?"dark:bg-slate-600 bg-blue-600 text-slate-200":""} my-0.5 py-2 px-4 flex justify-between items-center w-full text-left hover:bg-blue-500 hover:text-white hover:dark:bg-slate-700 rounded-lg`}>
                                {category?.name}
                            </button>
            ...
            {isOpen && category?.children?.length > 0 && (
                <ul className="ml-4 pl-4 border-l border-blue-500 dark:border-slate-700">
                    {category?.children?.map((child) => (
                        <NestedDropdown key={child?.id} category={child} setCategoryId={setCategoryId} categoryId={categoryId}/>
                    ))}
                </ul>
            )}
        </li>
...
const CategoryManuAdmin = ({categories,setCategoryId,categoryId}) => {
